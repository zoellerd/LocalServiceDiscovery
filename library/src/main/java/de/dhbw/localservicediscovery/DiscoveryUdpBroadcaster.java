...
public class DiscoveryUdpBroadcaster extends AsyncTask<Void, Void, Void> {

    public static final int UDP_PORT = 9996;
    public static final String LOG_TAG = "DHBW UdpBroadcaster";
    ...
    public DiscoveryUdpBroadcaster(Context context, AtomicBoolean gotOwnDatagram){
    ...
    protected Void doInBackground(Void... params){
        // gotOwnDatagram gets changed by DiscoveryUdpListener. if true, we can be sure we're listening
        do{
            try {
                Thread.sleep(50);
    ...
    public void sendDiscovery(InetAddress toAddr, boolean so_broadcast){
        ...
        random.nextBytes(DiscoveryUdpBroadcaster.last_random_bytes);

        try {
            outputStream.write(DiscoveryUdpBroadcaster.last_random_bytes);
    ...
    public static byte[] getLastRandomBytes(){
        return Arrays.copyOf(DiscoveryUdpBroadcaster.last_random_bytes, DiscoveryUdpBroadcaster.last_random_bytes.length);
    }
